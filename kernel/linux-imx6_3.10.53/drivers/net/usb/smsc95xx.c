...
static int smsc_mac_addr_setup(char *mac_addr)
{
	char *ptr, *p = mac_addr;
	unsigned long tmp; 
	int i = 0, ret = 0; 
...
__setup("smsc_mac=", smsc_mac_addr_setup);
...
static void smsc95xx_init_mac_address(struct usbnet *dev)
{
	...
	if (is_valid_ether_addr(macaddr)) {
		dev->net->dev_addr = macaddr;
		netif_dbg(dev, ifup, dev->net, "MAC address read from MAC IC\n");
		return;
	}
